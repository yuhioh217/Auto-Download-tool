...
const ftp = (conf, tag, ssl, cb) => {
	...
	Config.ReadConfig(conf, (err, configObj) => {
		if(err){
			...
			if(cb){
				return cb(err,'Read ' + fname + ' Config file failed');
			}
		}else{
			...
			c.on('ready', () => {
				c.list(basic_path, (err, list) => {
					if (err){
						if(cb){
							c.end();
							return cb(err,"Read list failed");
						}
					}
					...
					if(!isExist){
						c.mkdir(basic_path + tag, (err) => {
							...
							if(cb){
								c.end();
								return cb(null,"Create successfully.");
							}
						})
					}else{
						...
						if(cb){
							c.end();
							return cb(null,"Directory already existed");
						}
					}		
			...
			sClient.connect(options).then(() => {
				...
				if(!isExist){
					...
					sClient.connect(options).then(() => {
						...
						if(cb){
							sClient.end();
							return cb(null,"[mkdir.js] Create directory \"" + basic_path + tag + "\" successfully.");
						}
					}).catch((err) => {
						console.log(err);
						if(cb){
							sClient.end();
							return cb(err,"[mkdir.js] Create failed.");
						}
					});
				}else{
					if(cb){
						sClient.end();
						return cb(null,"[mkdir.js] Directory already existed. End the create stage.");
					}
				}
			}).catch((err) => {
				//console.log(err);
				if(cb){
					sClient.end();
					return cb(err,"[mkdirp.js] Read SFTP list failed")
				}
			});
		}
	});
	...
	sftp : (conf, tag, cb) => {
		...
		Config.ReadConfig(conf, (err,configObj) => {
			if(err){
				console.log('[download.js] Read SFTP config file failed.');
				if(cb){
					return cb(err,"Read SFTP config file failed.");
				}
			}else{
				console.log(tag);
