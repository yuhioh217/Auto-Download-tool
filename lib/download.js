...
const downloadSFTP = async(file, fnum, tmpdir, options, callback) =>{
	var dlf = '';
	...
	await sClient.connect(options).then(()=>{
		...
		stream.pipe(output.on('finish',()=>{
			...
			if(callback){
				sClient.end();
				callback(fnum,'downlaod file : ' + dlf + ' complete!');
			}
		}));
	}).catch((err)=>{
		console.log(err);
...
module.exports = {
	...
	ftp : (conf, FWversion, tags, tmpdir, cb) => {   //FTP downlaod API
		console.log(tags);
	...
	sftp : (conf, FWversion, tags, tmpdir, cb) => {
					...
					if(err){
						...
						getDirectoresSFTP(sClient, basic_path + tag, dl, options, (err, dl)=>{
							for(var i = 0;i < dl.length;i++){
								...
								downloadSFTP(dl[i], i,tmpdir, options,(fnum, msg)=>{
									console.log(msg);

									if(fnum == dl.length-1){
										if(cb){
											return cb(null,'SFTP finish');
										}
									}
								});
								//console.log(i + " " + dl.length-1);
